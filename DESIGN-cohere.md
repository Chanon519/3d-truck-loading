---
version: alpha
name: Cohere-navy-design-analysis
description: Cohere-inspired navy system adapted for a controlled enterprise logistics interface built from stark white editorial space, deep navy product bands, cool mineral surfaces, rounded media cards, and a distinctive type split between monospaced-feeling display headlines and precise Unica77 UI text.
...
This navy adaptation keeps Cohere's sober enterprise command-center restraint while shifting the brand system toward logistics, fleet, depot, and dispatch workflows. The home page opens on a huge typographic declaration over a white canvas, then uses photography, dark product mockups, trust logos, and generous empty space to make AI infrastructure feel controlled rather than speculative. Product pages invert the tone into deep navy, maritime-blue, or blue-black bands, while blog and research pages move toward publishing-system clarity: large filters, thin rules, dense lists, and pale technical backgrounds.

What makes the system distinctive is the mix of austere black-and-white UI with bursts of tactile brand imagery. The interface avoids decorative chrome; color arrives through photography, abstract 3D media, safety-orange taxonomy chips, blue action links, and dark navy product environments. Cards are rounded but not cute. Type is large, tight, and almost monospaced in spirit, creating a research-lab cadence across marketing, product, and editorial surfaces.

**Key Characteristics:**
- Monumental display headlines with very tight line height and negative tracking.
- White editorial canvases interrupted by deep navy, maritime-blue, and image-led CTA bands.
...
- Blog and research surfaces with prominent safety-orange taxonomy chips, long rule-separated lists, and search fields.

## Colors

### Brand & Accent

- **Blue Black** (`#020617`): Announcement bar, highest-contrast text, and the global brand anchor.
- **Navy Primary** (`#0F172A`): Primary CTA buttons, dark footer, and deep UI cards.
- **Deep Maritime Navy** (`#0B2A3A`): Product hero bands, command sections, dispatch panels, and logistics feature bands.
- **Dark Operations Navy** (`#071A33`): Security, fleet, route-planning, and operations-oriented solution bands.
- **Action Blue** (`#2563EB`): Editorial links, pagination, route links, and secondary action emphasis.
- **Safety Orange** (`#F97316`): Taxonomy chips, status emphasis, dispatch markers, optimization highlights, and warm product markers.
- **Soft Orange** (`#FDBA74`): Pale chip borders, segmented article-label details, and secondary warning-like accents.

### Surface & Background

- **Canvas White** (`#ffffff`): Dominant page background and form/card surface.
- **Cool Stone** (`#EEF2F6`): Product cards, testimonial placeholders, and cool neutral surface blocks.
- **Pale Aqua Wash** (`#EAF6F8`): Section backdrop behind stacked dark capability panels.
- **Pale Blue Wash** (`#EEF6FF`): Blog CTA surface behind abstract 3D imagery.
- **Card Border** (`#EDF2F7`): Softest card containment line.

### Text & Rules

- **Ink** (`#111827`): Default body text and most link text on light backgrounds.
- **Muted Slate** (`#8A94A6`): Footer links, dates, metadata, and de-emphasized labels.
- **Slate** (`#64748B`): Research separators and tertiary text.
- **Hairline** (`#D6DEE8`): Standard list rules and section dividers.
- **Border Light** (`#E2E8F0`): Secondary divider and utility rule.

### Semantic

- **Focus Blue** (`#3B82F6`): Keyboard focus and ring color.
- **Form Focus Cyan** (`#0EA5E9`): Focus border for text inputs.
- **Error Red** (`#DC2626`): Validation and destructive-state color.

### Gradient System

This navy adaptation does not use gradients as a generic UI fill. Gradients and color fields are media-led: abstract 3D hero imagery, deep blue technical particle fields, orange dispatch markers, and dark navy-to-blue-black product environments. Keep UI surfaces flat; reserve gradient richness for large media panels and CTA image bands.
...
- Editorial pages can use safety-orange chips and blue links, but the base typography remains navy-black and measured.
...
| Bordered | 1px `#D6DEE8`, `#E2E8F0`, or dark translucent rules | Research rows, forms, pale cards, footer inputs |
| Media Lift | Rounded image or video over contrasting section color | Hero photo cards, product videos, CTA imagery |
| Dark Product Field | Deep navy or maritime-blue full-width band | Command, routing, fleet, financial services, security sections |
...
Deep navy or maritime-blue full-width section used for product capabilities, dispatch operations, security claims, and feature breakdowns. Text turns white; cards use darker translucent surfaces, pale borders, and abstract line illustrations.
...
Large safety-orange taxonomy chip used on the blog index. Active chips invert to orange fill with dark text; inactive chips use orange outline and pale fill. Typography is oversized relative to typical filters, making the taxonomy a hero-level control.
...
Rounded white form panel set against deep navy or cool stone sections. Inputs are rectangular with thin gray borders, 12px-16px padding, and compact labels/placeholders. Submit uses the same near-black pill style as primary CTAs.

### **`footer-newsletter`**

Dark footer subscription block with safety-orange label, white headline, muted legal microcopy, a single-line email field, and arrow submit marker. Footer columns use white section labels and muted links.

## Do's and Don'ts

### Do

- Use white canvas as the default surface; introduce navy, deep maritime blue, or blue-black as full-width product bands.
- Keep primary CTAs pill-shaped and near-black on light surfaces.
- Use 22px radius on major media cards and placeholders.
- Use safety orange for taxonomy, dispatch emphasis, and small warm accents, not as the broad surface system.
...
- Do not turn orange or blue into broad decorative surface colors.
...
1. Start from a white canvas or a full-width navy/maritime-blue band; avoid mid-tone page backgrounds unless the screenshot shows a specific CTA/form section.
